...
SCHEMA_VERSION = "phase2d.body_enriched_items.v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build experimental items with Malay Mail article-body excerpts and Astro Awani RSS fallback."
    )
    parser.add_argument("--limit", type=int, default=10, help="Maximum unique article URLs to include.")
    parser.add_argument(
        "--output",
        default="/tmp/body_enriched_items_test.json",
    ...
    parser.add_argument(
        ...
        type=float,
        default=20.0,
        help="Per-article newspaper3k download timeout for Malay Mail items.",
    )
    parser.add_argument(
        "--excerpt-chars",
        type=int,
        default=1200,
        help="Maximum characters to keep from fetched article body text.",
    )
...
def entry_summary(entry: Any) -> str:
    if not hasattr(entry, "get"):
        return ""
    return clean_html_text(entry.get("summary", entry.get("description", "")))
            ...
            source_candidates.append(
                {
                    ...
                    "rss_summary": entry_summary(entry),
                    "body_policy": source["body_policy"],
                }
            )
        feed_candidate_buckets.append(source_candidates)
...
def base_item(candidate: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "rss_summary": candidate["rss_summary"],
...
def fetch_article_body(
    ...
    try:
        ...
        if not text:
            item["body_error"] = "empty_text"
            return item
        item["content_source"] = ARTICLE_BODY
        item["body_fetched"] = True
        item["body_text_length"] = len(text)
        item["body_excerpt"] = excerpt(text, excerpt_chars)
        return item
...
def build_item(
    ...
    if candidate.get("body_policy") == "try_article_body":
        return fetch_article_body(candidate, Article, Config, timeout_sec, excerpt_chars)

    item = base_item(candidate)
    item["body_error"] = "skipped_by_policy"
    return item
...
def build_payload(
    ...
    article_body_count = sum(1 for item in items if item.get("content_source") == ARTICLE_BODY)
    rss_fallback_count = sum(1 for item in items if item.get("content_source") == RSS_FALLBACK)
    return {
        "schema_version": SCHEMA_VERSION,
        "generated_at": datetime.now(TIMEZONE).isoformat(),
        "policy": {
            "article_body_feeds": ["Malay Mail Malaysia", "Malay Mail Money"],
            "rss_fallback_feeds": ["Astro Awani National"],
        ...
        "counts": {
            ...
            "items": len(items),
            "article_body": article_body_count,
            "rss_fallback": rss_fallback_count,
            "body_fetch_failed": sum(
                1
                for item in items
                if item.get("feed", "").startswith("Malay Mail") and item.get("content_source") == RSS_FALLBACK
            ),
        },
        "items": items,
    }
...
def main() -> int:
    ...
    for index, candidate in enumerate(candidates, start=1):
        item = build_item(candidate, Article, Config, args.timeout_sec, args.excerpt_chars)
        items.append(item)
        detail = item["content_source"]
        if item.get("body_error"):
            detail += f" ({item['body_error']})"
        print(f"item {index}: {candidate['feed']} -> {detail}")

    payload = build_payload(feed_results, items, args.limit, args.output, args.timeout_sec, args.excerpt_chars)
