...
SCHEMA_VERSION = "phase2d.groq_rss_vs_filtered_context.v1"
...
conclusionではRM1.7bなどの金額略記を無理に日本円・億などへ換算せず、原文表記を優先してください。
what_happenedは入力にある事実だけで、最大2件にしてください。
life_impactは入力から言える範囲に留め、推測を盛らないでください。
事故・犯罪・死亡記事が入力に含まれる場合でも、被害詳細を増やしすぎないでください。
分からない場合は控えめな背景情報として表現してください。
出力はJSON objectのみです。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare Groq summaries for RSS-only context vs Phase 2D.4 filtered context."
    )
    parser.add_argument("--json-input", required=True, help="Path to Phase 2D.4 filtered context JSON.")
    parser.add_argument(
        "--output",
        default="/tmp/groq_rss_vs_filtered_context_comparison.json",
        help="Path to write Groq A/B comparison JSON.",
    )
    parser.add_argument(
        "--memo-output",
        default="/tmp/groq_rss_vs_filtered_context_comparison_memo.md",
        help="Path to write Markdown observation memo.",
    )
    parser.add_argument("--limit", type=int, default=3, help="Maximum items to compare.")
    ...
    parser.add_argument(
        "--include-all",
        action="store_true",
        help="Compare rss_only/rss_fallback policy items too. Defaults to use_body items only.",
    )
...
def groq_user_payload(item: dict[str, Any], variant: str, context: str) -> dict[str, Any]:
    return {
        ...
        "body_excerpt_policy": item.get("body_excerpt_policy", ""),
        "body_excerpt_reason": item.get("body_excerpt_reason", ""),
...
def comparison_items(data: dict[str, Any], limit: int, include_all: bool) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
    for item in input_items(data):
        policy = item.get("body_excerpt_policy")
        if not include_all and policy != USE_BODY:
            continue
        items.append(item)
        if len(items) >= limit:
            break
...
def filtered_context_key(item: dict[str, Any]) -> str:
    if item.get("selected_context") == WITH_BODY_EXCERPT:
        return "with_body_excerpt"
    return "rss_only"


def filtered_context_value(item: dict[str, Any]) -> str:
    return text_value(item.get(filtered_context_key(item)))
...
def build_result_item(
    ...
    selected_context = filtered_context_key(item)
    selected_context_text = filtered_context_value(item)
    return {
        ...
        "body_excerpt_used": selected_context == WITH_BODY_EXCERPT,
        "rss_only_result": run_variant(item, "rss_only", rss_only, api_key, model, temperature),
        "filtered_context_result": run_variant(
            item,
            selected_context,
...
def build_payload(
    ...
    targets = comparison_items(source_data, limit, include_all)
    items = [build_result_item(item, api_key, model, temperature) for item in targets]
    ok_results = sum(
        1
        for item in items
        for key in ("rss_only_result", "filtered_context_result")
    ...
    source_items = input_items(source_data)
    return {
        ...
        "counts": {
            "available_items": len(source_items),
            "available_use_body_items": sum(1 for item in source_items if item.get("body_excerpt_policy") == USE_BODY),
...
def render_item(item: dict[str, Any]) -> list[str]:
    ...
    lines.extend(render_summary_result("B: 本文使用制限あり", item.get("filtered_context_result")))
    lines.extend(
        [
            "### 観察メモ",
            "",
            ...
            "- 次回確認: ",
            "",
            "見る観点: 本文excerptで具体性が増えたか、余計な詳細が増えていないか、生活影響が入力範囲に留まっているか。",
            "",
        ]
    )
    return lines
...
def render_memo(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Groq RSS summaryのみ vs 本文使用制限あり A/B観察メモ",
        "",
        ...
        f"- include_all：{safe_text(payload.get('include_all'))}",
        f"- available_items：{safe_text(counts.get('available_items', 0))}",
        f"- available_use_body_items：{safe_text(counts.get('available_use_body_items', 0))}",
...
def main() -> int:
    ...
    print(
        "groq filtered-context comparison: "
