...
from malaysia_groq_fallback_policy import safe_json_render_fallback_summary_for_item
...
def normalize_fallback_summaries_for_json_render(
    ...
    render_decisions: list[RenderDecision] | None = None,
) -> dict[str, Any]:
    """Apply the merge fallback cleanup to non-accepted items before JSON rendering."""
    if render_decisions is not None:
        return apply_json_render_decisions(data, render_decisions)
...
def normalize_entry_candidate_summaries_for_observation(
    ...
    if render_decisions is not None:
        return apply_entry_render_decisions(data, render_decisions)
